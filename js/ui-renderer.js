const UIRenderer = {
    ...
    // Tab Cửa hàng (Có Cảnh báo Hợp đồng thuê)
   // Tab 1: Cửa hàng (FIX: Đã có Map Link + Cảnh báo thuê + Đủ cột địa bàn)
    renderStoresTable(data) {
        ...
        tbody.innerHTML = data.map((item, idx) => {
            ...
            <tr class="${rowClass} border-b hover:bg-slate-50">
                ...
                <td class="p-3 border-b text-sm text-slate-700">${item.lienCum}</td>
                <td class="p-3 border-b text-sm text-slate-500">${item.cum}</td>
                ...
                <td class="p-3 border-b text-center">
                    <button class="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-full admin-only"><i data-lucide="edit-3" class="w-4 h-4"></i></button>
                </td>
            </tr>
            `;
        }).join('');
    ...
    renderGDVTable(data) {
        ...
        tbody.innerHTML = data.map(item => `
            <tr class="${this.getRowClass(item.trangThai)} border-b transition">
                ...
                <td class="p-3 text-xs">${item.lienCum}</td>
                <td class="p-3 text-xs">${item.cum}</td>
    ...
    renderIndirectTable(data) {
        ...
        tbody.innerHTML = data.map((item, idx) => `
            <tr class="bg-white border-b hover:bg-slate-50 transition">
                ...
                <td class="p-3 text-center">
                    <button class="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-full admin-only">
    ...
    renderKPIStructureTable(data) {
        ...
        tbody.innerHTML = data.map(item => `
            <tr class="bg-white border-b hover:bg-slate-50">
                ...
                <td class="p-3 text-center">
                    <button class="admin-only text-slate-400 hover:text-blue-600"><i data-lucide="edit-3" class="w-4 h-4"></i></button>
                </td>
            </tr>
        `).join('');
        lucide.createIcons();
    ...
    renderKPIActualTable(data, structure) {
        ...
        tbody.innerHTML = data.map(row => {
            let rowHtml = `<tr class="bg-white border-b hover:bg-slate-50"><td class="p-3 font-bold text-slate-600">${row.lienCum}</td><td class="p-3 text-sm">${row.cum}</td><td class="p-3 font-medium text-blue-700">${row.phuong}</td>`;
            structure.forEach(kpi => {
                if (kpi.active) {
    ...
    renderKPIUserLogs(data) {
        ...
        tbody.innerHTML = data.map(item => `
            <tr class="bg-white border-b hover:bg-slate-50">
                <td class="p-3 text-center">${item.stt}</td>
                <td class="p-3 text-sm">${item.lienCum}</td>
                <td class="p-3 text-sm">${item.cum}</td>
    ...
    renderPlanningTable(staffList) {
        ...
        tbody.innerHTML = staffList.map((nv, idx) => `
            <tr class="bg-white border-b hover:bg-slate-50">
                ...
                <td class="p-3 text-xs text-slate-500">${nv.cum || nv.lienCum}</td>
    ...
    renderDashboardSummary(clusters, stores, gdvs, sales, indirect, bts) {
        ...
        const summaryData = clusters.map(lc => {
            const lcName = lc.tenLienCum;
            return { 
                lcName, 
                countCum: lc.cums.length,
                countStore: stores.filter(i => i.lienCum === lcName).length,
                countGDV: gdvs.filter(i => i.lienCum === lcName).length,
                countSale: sales.filter(i => i.lienCum === lcName).length,
                countAgency: indirect.filter(i => i.lienCum === lcName).length,
                countBTS: bts.filter(i => i.lienCum === lcName).length,
                rawData: lc 
            };
        });
        ...
        tbody.innerHTML = summaryData.map((item, idx) => `
            <tr class="bg-white border-b hover:bg-slate-50">
                <td class="p-3 text-center">${idx + 1}</td>
                <td class="p-3 font-bold text-blue-700">${item.lcName}</td>
                ...
                <td class="p-3 text-center">
                    <button class="text-blue-600 hover:underline text-xs flex items-center justify-center w-full" 
                        onclick='UIRenderer.showClusterDetail(${JSON.stringify(item.rawData)})'>
    ...
    renderDashboardCharts(kpiData, instances) {
        ...
        const labels = [...new Set(kpiData.map(d => d.lienCum))];
        const dataPlan = labels.map(l => 600); // Mock Plan
        const dataActual = labels.map(l => kpiData.filter(d => d.lienCum === l).reduce((sum, item) => sum + (item.KPI_DT || 0), 0));
